...
pub async fn handle_api_pairing_devices(
    ...
    // paired_devices method not implemented on PairingGuard
    // Return token list as placeholder
    let tokens = state.pairing.tokens();
    let devices: Vec<serde_json::Value> = tokens
        .iter()
        .enumerate()
        .map(|(i, token)| {
            serde_json::json!({
...
pub async fn handle_api_pairing_device_revoke(
    ...
    // revoke_device method not implemented on PairingGuard
    // This endpoint is not functional until PairingGuard supports device revocation
    (
        StatusCode::NOT_IMPLEMENTED,
        Json(serde_json::json!({"error": "Device revocation not implemented"})),
    )
        .into_response()
}
