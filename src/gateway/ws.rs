...
fn refresh_ws_history_system_prompt_datetime(_history: &mut [ChatMessage]) {
    // DateTime is now included in the system prompt when it's generated.
    // No runtime refresh needed since DateTimeSection generates current time.
}
...
mod tests {
    ...
    #[test]
    fn refresh_ws_history_system_prompt_datetime_is_noop() {
        let mut history = vec![
            ChatMessage::system("## Current Date & Time\n\n2000-01-01 00:00:00 (UTC)\n"),
            ChatMessage::user("hello"),
        ];
        refresh_ws_history_system_prompt_datetime(&mut history);
        // Function is now a no-op since datetime is generated fresh each time
        assert_eq!(
            history[0].content,
            "## Current Date & Time\n\n2000-01-01 00:00:00 (UTC)\n"
        );
