...
fn truncate_sample(raw: &str) -> String {
    ...
    // Find the largest valid UTF-8 char boundary at or before the limit
    let mut idx = MAX_ALERT_SAMPLE_CHARS.min(raw.len());
...
fn command_identity(command: &str) -> String {
    ...
    if lowered.len() <= 64 {
        ...
        // Find the largest valid UTF-8 char boundary at or before 64
        let mut boundary = 64.min(lowered.len());
        while boundary > 0 && !lowered.is_char_boundary(boundary) {
            boundary -= 1;
        }
