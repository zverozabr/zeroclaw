    ...
    for iteration in 0..max_iterations {
        ...
        // often generate a redundant plain-text summary or a malformed tool call.
        {
            ...
            if let Some(output) = terminal_output {
                ...
                if let Some(ref tx) = on_delta {
                    let _ = tx.send(DraftEvent::Clear).await;
                    let _ = tx.send(DraftEvent::Content(output.clone())).await;
                }
                return Ok(output);
            }
        }
    ...
    async fn run_tool_call_loop_respects_per_tool_call_cap() {
        ...
        impl Tool for CappedTool {
            ...
            async fn execute(
                ...
                Ok(crate::tools::ToolResult {
                    success: true,
                    output: format!("executed:{value}:call{n}"),
                    error: None,
                })
            }
        }
        ...
        // The remaining 1 (4 total - 3 cap) should have produced skip messages.
        ...
        assert_eq!(
            skip_count, 1,
            "Expected 1 skip message for capped calls, but found {skip_count}"
        );
    }
