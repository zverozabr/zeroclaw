...
// TODO: detection and history modules removed
// use detection::{DetectionVerdict, LoopDetectionConfig, LoopDetector};
use execution::{
    execute_tools_parallel, execute_tools_sequential, execute_tools_staged, is_search_phase_tool,
    is_terminal_tool, should_execute_tools_in_parallel, ToolExecutionOutcome,
};
// #[cfg(test)]
...
// TODO: Stub definitions for removed detection and history modules
#[derive(Debug, Clone, Default)]
struct LoopDetectionConfig {
    no_progress_threshold: usize,
    ping_pong_cycles: usize,
...
impl LoopDetector {
    fn new(config: LoopDetectionConfig) -> Self {
        Self { _config: config }
    }

    fn report_tool_call(&mut self, _tool_name: &str, _success: bool) -> DetectionVerdict {
        DetectionVerdict::Continue
    }

    fn record_call(&mut self, _tool_name: &str, _args: &str, _output: &str, _success: bool) {
        // Stub: no-op
    }

    fn check(&self) -> DetectionVerdict {
...
impl TurnBuffer {
    ...
    fn drain_for_extraction(&mut self) -> Vec<(String, String)> {
        // Stub: return empty
        Vec::new()
    }

    fn push(&mut self, _user: &str, _assistant: &str) {
        // Stub: no-op
    }
    ...
    fn mark_extract_success(&mut self) {
        // Stub: no-op
    }

    fn needs_compaction_fallback(&self) -> bool {
        // Stub: never needs compaction
        false
    }
}
...
async fn auto_compact_history(
    ...
    _memory: Option<&dyn Memory>,
    _session_id: Option<&str>,
    _post_turn_active: bool,
) -> Result<(bool, bool)> {
    // Stub: no compaction, no flush
    Ok((false, false))
}

fn trim_history(_history: &mut Vec<ChatMessage>, _max_turns: usize) {
    // Stub: no-op
}
...
async fn extract_facts_from_turns(
    _provider: &dyn Provider,
    _model: &str,
...
/// Hard cap on tool calls executed from a single LLM turn.
/// Prevents resource exhaustion when the model emits an unreasonable
/// number of calls (observed: 100+ search calls with nonsense keywords).
const MAX_TOOL_CALLS_PER_TURN: usize = 15;
...
pub async fn run_tool_call_loop(
    ...
    for iteration in 0..max_iterations {
        ...
        {
            let preview: String = last_user_content.chars().take(300).collect::<String>();
        ...
        // --- Hard cap: prevent tool-call explosion (single enforcement point) ---
        let tool_calls = if tool_calls.len() > MAX_TOOL_CALLS_PER_TURN {
            let original_count = tool_calls.len();
            let kept_names: Vec<&str> = tool_calls
                .iter()
                .take(MAX_TOOL_CALLS_PER_TURN)
                .map(|c| c.name.as_str())
                .collect();
            let dropped_names: Vec<&str> = tool_calls
                .iter()
            ...
            tracing::warn!(
                ...
                "tool_call_explosion: LLM emitted {} tool calls, capping to {}",
                original_count,
                MAX_TOOL_CALLS_PER_TURN,
            );
            tracing::info!(
                ?kept_names,
                "tool_call_explosion: keeping first {} calls",
                MAX_TOOL_CALLS_PER_TURN,
            );
            tracing::info!(
                ?dropped_names,
                "tool_call_explosion: dropping {} calls",
                dropped_names.len(),
            );
            runtime_trace::record_event(
                "tool_call_explosion_capped",
                Some(channel_name),
                Some(provider_name),
                Some(active_model.as_str()),
                Some(&turn_id),
                Some(false),
                Some(&format!(
                    "LLM requested {} tool calls; capped to {}",
                    original_count, MAX_TOOL_CALLS_PER_TURN
                )),
                serde_json::json!({
                    "iteration": iteration + 1,
                    "requested": original_count,
            ...
            .zip(progress_indices.iter())
        {
            ...
            if let Some(hooks) = hooks {
                ...
                // TODO: run_tool_result_persist method removed, skipping result transformation
                // match hooks
        ...
        // Check if a terminal tool (submit_contacts) produced a meaningful result.
        // If so, return it directly without asking the LLM for another turn —
        // this prevents the model from writing a redundant plain-text summary.
        let terminal_output: Option<String> = ordered_results
            .iter()
            .flatten()
            .find(|(name, _, outcome)| {
                is_terminal_tool(name)
                    ...
                    && !outcome.output.contains("Не найдено подходящих контактов")
                    && !outcome.output.contains("Skipped duplicate")
                    && !outcome.output.contains("Отправлено:")
            })
            .map(|(_, _, outcome)| outcome.output.clone());
        ...
        // If submit_contacts produced a formatted result, use it as the final reply.
        if let Some(ref terminal_reply) = terminal_output {
            tracing::info!(
                output_len = terminal_reply.len(),
                "terminal_tool.early_return submit_contacts produced reply, skipping LLM turn"
            );
            if let Some(ref rec) = session_recorder {
                if let Some(ref dir_str) = session_report_dir {
                    rec.finalize_and_write(std::path::Path::new(dir_str), session_start);
            ...
            return Ok(terminal_reply.clone());
        }
...
pub async fn run(
    ...
    // ── Skill tools (workspace skills → native function-calling tools) ──
    let skills = crate::skills::load_skills_with_config(&config.workspace_dir, &config);
    let skill_tools = crate::skills::create_skill_tools(&skills, Arc::clone(&security));
    if !skill_tools.is_empty() {
        tracing::info!(count = skill_tools.len(), "Skill tools added (CLI agent)");
    ...
    // TODO: create_runner_from_config doesn't exist, using empty runner
    let configured_hooks = crate::hooks::HookRunner::new();
    let effective_hooks = Some(hooks.unwrap_or(&configured_hooks));
    ...
    if let Some(msg) = message {
        ...
        loop {
            ...
            let read_result = {
                ...
                std::io::BufRead::read_until(&mut locked, b'\n', &mut raw)
            };
            match read_result {
                Ok(0) => {
            ...
            // TODO: refresh_prompt_datetime removed
            // if let Some(system_message) = history.first_mut() {
            //     if system_message.role == "system" {
...
{"name": "file_write", "arguments": {"path": "hello.py", "content": "print('hello')"}}"#;
    ...
    fn trim_history_preserves_system_prompt() {
        ...
        // trim_history is now a no-op stub, so history remains unchanged
...
Final answer."#;
    ...
    /// Local helper duplicating Dispatcher::strip_think_tags (private) for test use.
    ...
    fn trim_history_with_no_system_prompt() {
        ...
        trim_history(&mut history, DEFAULT_MAX_HISTORY_MESSAGES);
        // trim_history is now a no-op stub, so history remains unchanged
        assert_eq!(history.len(), DEFAULT_MAX_HISTORY_MESSAGES + 20);
    }
    ...
    #[test]
    fn max_tool_calls_per_turn_bounds() {
        assert!(MAX_TOOL_CALLS_PER_TURN >= 1, "cap must be positive");
        assert!(MAX_TOOL_CALLS_PER_TURN <= 50, "cap must be reasonable");
        assert!(
            MAX_TOOL_CALLS_PER_TURN < DEFAULT_MAX_TOOL_ITERATIONS * 5,
            "per-turn cap must be meaningful within iteration budget"
        );
    }
...
Let me check the result."#;
    ...
    fn trim_history_removes_oldest_non_system() {
        ...
        assert_eq!(history[0].role, "system");
        assert_eq!(history[1].content, "old msg");
    }
