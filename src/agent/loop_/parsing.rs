...
/// Remove `<think>...</think>` blocks from model output (e.g. Qwen reasoning tags).
...
pub(super) fn parse_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(cleaned.trim()) {
    ...
    if let Some((minimax_text, minimax_calls)) = parse_minimax_invoke_calls(&cleaned) {
    ...
    if calls.is_empty() {
        ...
        for cap in MD_TOOL_CALL_RE.captures_iter(&cleaned) {
    ...
    if calls.is_empty() {
        ...
        for cap in MD_TOOL_NAME_RE.captures_iter(&cleaned) {
            let full_match = cap.get(0).unwrap();
            let before = &cleaned[last_end..full_match.start()];
            if !before.trim().is_empty() {
                md_text_parts.push(before.trim().to_string());
