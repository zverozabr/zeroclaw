...
## ВАЖНО: Telegram ограничения
- ЧИТАТЬ сообщения можешь из ЛЮБОГО чата (через Telethon от имени пользователя)
- ПИСАТЬ можешь ТОЛЬКО в чат с пользователем (через Bot API)
- НЕ ПИШИ в чужие группы — бот там не добавлен, это не сработает
- Когда просят "поищи в чате X" → ЧИТАЙ через Telethon, результаты ОТВЕЧАЙ пользователю
